...
public class WebLogAspect {
     ...
     *
     * @annotation(webLog)这种写法，注解的生命周期必须定义为RUNTIME
     *         1.SOURCE 仅存在于编译器处理期间，编译器处理完之后，就没用了
     *         2.CLASS  默认的，将注解信息存储于类对应的class中
     *         3.RUNTIME 将注解信息存于class文件中，可由JVM读入，在运行期间通过反射可以访问
     */
